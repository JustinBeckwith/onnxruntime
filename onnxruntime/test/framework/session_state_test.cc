...
TEST(SessionStateSessionOptionsTest, LayeringRulesParsing) {
  // Test empty string
  {
    auto rules = LayeringRules::FromConfigString("");
    EXPECT_TRUE(rules.rules.empty());
  }

  // Test simple valid string
  {
    auto rules = LayeringRules::FromConfigString("EP1(Annotation1)");
  ...
  {
    auto rules = LayeringRules::FromConfigString("EP1(Annotation1, Annotation2)");
  ...
  {
    auto rules = LayeringRules::FromConfigString("EP1(Annotation1); EP2(Annotation2)");
    ASSERT_EQ(rules.rules.size(), 2u);
    EXPECT_EQ(rules.rules[0].device, "EP1");
    EXPECT_EQ(rules.rules[0].annotation, "Annotation1");
    EXPECT_FALSE(rules.rules[0].prefix_match);
  ...
  {
    auto rules = LayeringRules::FromConfigString("  EP1  (  Annotation1  ,  =Annotation2  )  ;  EP2  (  Annotation3  )  ");
  ...
  {
    auto rules = LayeringRules::FromConfigString("InvalidEntry; EP1(); (Annotation); EP1(Annotation");
    EXPECT_TRUE(rules.rules.empty());
  }
}
